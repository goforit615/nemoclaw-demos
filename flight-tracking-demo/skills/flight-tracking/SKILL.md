...
description: "Live aircraft tracking, airport lookup, and interactive map control for the FlightOps map UI at http://127.0.0.1:18890. Use whenever the user asks about live air traffic, what is flying near an airport, inbound patterns, unusual squawks, METAR weather, NAS status / ground stops, ARTCC boundaries, OR wants the map driven (fly to an airport, draw arcs, highlight/track a flight, RECOLOUR planes, toggle a layer, filter by phase or squawk, switch to 3D, change METAR colour mode). EXEC-READY COPY-PASTE RECIPES — pick the one matching user intent and run it verbatim, substituting the airport / mode / flight token. The exact JSON body matters: each endpoint has a different field name, do NOT guess. (A) 'go to <X> and analyse [traffic]' → run TWO calls: `curl -fsS --max-time 60 -X POST -H 'Content-Type: application/json' -d '{\"target\":\"<X>\",\"zoom\":9}' 'http://127.0.0.1:18890/api/map/goto'` then `curl -fsS --max-time 60 'http://127.0.0.1:18890/api/analyze?airport=<X>&radius_km=80'`. NOTE: goto's field is `target`, NOT `airport`. (B) 'show arcs into <X>' → ONE call: `curl -fsS --max-time 60 -X POST -H 'Content-Type: application/json' -d '{\"airport\":\"<X>\",\"radius_km\":80}' 'http://127.0.0.1:18890/api/map/arcs'`. NOTE: arcs' field IS `airport` (different from goto). (C) 'recolour planes by <altitude|vrate|squawk|phase>' → ONE call: `curl -fsS --max-time 60 -X POST -H 'Content-Type: application/json' -d '{\"mode\":\"<altitude|vrate|squawk|phase>\"}' 'http://127.0.0.1:18890/api/map/color'`. (D) 'find <plane matching X> and track it' → TWO calls: `curl -fsS --max-time 60 'http://127.0.0.1:18890/api/flights/find?departing=<X>&limit=5'` then pick the first id and `curl -fsS --max-time 60 -X POST -H 'Content-Type: application/json' -d '{\"flight\":\"<id>\",\"zoom\":10}' 'http://127.0.0.1:18890/api/map/track'`. (E) 'toggle layer <L> on/off' → `curl -fsS --max-time 60 -X POST -H 'Content-Type: application/json' -d '{\"layer\":\"<L>\",\"visible\":true}' 'http://127.0.0.1:18890/api/map/layer'` (layer ∈ flights|airports|arcs|trails|paths|weather|sua|classes|tfrs|runways|taxiways|obstacles|ats|metar|nas|artcc|navaids). (F) 'is <airport> delayed / GDP / ground stop?' → `curl -fsS --max-time 60 'http://127.0.0.1:18890/api/nas/airport/<X>'`. (G) 'weather at <X> / METAR' → `curl -fsS --max-time 60 'http://127.0.0.1:18890/api/weather/metar?bbox=-77,38,-76,39'`. (H) 'what is <airport>' → `curl -fsS --max-time 60 'http://127.0.0.1:18890/api/airport/<X>'`. ANTI-PATTERNS — DO NOT: exec a bare URL without `curl` (bash will run it as a command); omit `--max-time 60`; use GET on a /api/map/* endpoint (they are POST-only — GET returns 405); guess JSON field names — every endpoint above lists its exact body inline; read files under /sandbox/.openclaw/skills/ (this frontmatter has everything); call gog/gmail/planet/wakeup or any OTHER skill (stay in flight-tracking); curl upstream OpenSky/FAA/AWC hosts directly (firewalled, Tier-1 host proxies handle them). HARD RULES: (1) ANY map change REQUIRES the matching POST FIRST, BEFORE describing the result. (2) Aircraft colour modes are EXACTLY four: phase | altitude | vrate | squawk (aliases 'elevation', 'flight level', 'rate of climb', 'emergency' accepted). Do NOT invent palettes. (3) Every /api/map/* response has `delivered` = browser tabs reached. If delivered >= 1 just confirm the change in ONE sentence and STOP — do NOT add 'if you don't see the change open the UI' or any other 'just in case' hedge. If delivered:0 say 'I issued the command but no map UI is connected; open http://127.0.0.1:18890 first' — do NOT claim the map updated. (4) If a call returns 405 or 422 you used the wrong method/body — re-read the recipe above and retry ONCE. If it still fails, say 'service unavailable' and stop — do NOT pivot to a different skill. (5) Inference is slow; complete in the FEWEST tool calls possible. The recipes above already give you the minimum."
---
