...
description: "Live aircraft tracking, airport lookup, and interactive map control for the FlightOps map UI at http://127.0.0.1:18890. Use this skill whenever the user asks about live air traffic, what is flying near an airport, inbound patterns, unusual squawks, METAR weather, NAS status / ground stops, ARTCC boundaries, OR wants the map driven in ANY way (fly to an airport, draw arcs, highlight a flight, RECOLOUR the planes, toggle a layer, filter by phase or squawk, switch to 3D, change METAR colour mode). COMMAND TEMPLATE — every exec call is EXACTLY one of these two forms, never anything else: GET: `curl -fsS --max-time 60 'http://127.0.0.1:18890/api/...'` POST: `curl -fsS --max-time 60 -X POST -H 'Content-Type: application/json' -d '<json>' 'http://127.0.0.1:18890/api/...'`. NEVER exec a bare URL (bash will try to run it as a command). NEVER omit --max-time (some endpoints take 5-10s and bash subshells will be killed). FAST PATH BY USER INTENT: (A) 'go to <airport> and analyse [traffic]' → POST /api/map/goto + GET /api/analyze?airport=X&radius_km=80 (both ~1s; analyse never uses /api/flights/find for this). (B) 'show arcs into <airport>' → POST /api/map/arcs (auto-tilts; no goto needed). (C) 'recolour by altitude/squawk/etc' → POST /api/map/color {\"mode\":\"...\"}. (D) 'find <plane> and track it' → GET /api/flights/find then POST /api/map/track. (E) 'what is at this airport / weather here' → GET /api/airport/X, GET /api/nas/airport/X, GET /api/weather/metar. ANTI-PATTERNS — DO NOT: read SKILL.md or any file under /sandbox/.openclaw/skills/ (everything you need is in this frontmatter); call gog/gmail/planet/wakeup or any OTHER skill (this is a flight-tracking query, stay in this skill); loop over /api/route/<cs> for live flights (use /api/flights/find?confirm_route=true server-side fan-out instead); curl opensky-network.org, aviationweather.gov, nasstatus.faa.gov, adsbdb.com, hexdb.io, tfr.faa.gov directly — they're firewalled (Tier-1 proxies them through the host). HARD RULES: (1) ANY request that changes what's drawn on the map REQUIRES issuing the matching POST /api/map/... call FIRST, BEFORE describing the result. Never claim 'I've updated the map' without issuing the POST this turn. (2) Aircraft colour scheme has EXACTLY FOUR fixed presets: phase | altitude | vrate | squawk. Aliases 'elevation', 'flight level', 'rate of climb', 'emergency' are accepted. Do NOT invent custom palettes; describe only what the preset shows: phase=orange family, altitude=mint→deep green ramp (NOT rainbow), vrate=violet↔magenta diverging, squawk=grey + red/amber for 7500/7600/7700. (3) EVERY /api/map/* response includes `delivered` = how many browser tabs received the broadcast. If delivered:0 say 'I issued the command but no map UI is connected; open http://127.0.0.1:18890 first', do NOT claim the map updated. (4) If a local call returns non-200 or empty, say 'data unavailable' or 'service unavailable' — do NOT invent a network-outage excuse and do NOT pivot to a different skill. (5) Other map controls: POST /api/map/metar-color (flt_cat|wind|temp|visibility), /api/map/layer, /api/map/filter, /api/map/view (free-form camera), /api/map/airspace3d, /api/map/highlight. Open the SKILL.md body only for field schemas you genuinely need."
---
